...
Responsável por autenticação, gerenciamento de despesas e categorias, relatórios financeiros, e documentação interativa via Swagger.
...
1. Uma **interface** define o contrato do validator (_Strategy_):
...
Cada validator pode lançar uma exceção (`ConflictException`, `NotFoundException`) para interromper a cadeia.
Para adicionar uma nova regra, basta criar uma classe que implemente a interface e registrá-la no array do módulo — sem alterar código existente (_Open/Closed Principle_).
...
O projeto usa migrações manuais do TypeORM (sem `synchronize`).
A última migração inclui um seed de dados iniciais. O fluxo de migrações é gerenciado pelos scripts:
